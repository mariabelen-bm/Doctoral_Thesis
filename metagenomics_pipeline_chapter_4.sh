...
# Trimming refers to the process of cleaning up and preparing DNA, RNA or protein sequences by removing unwanted parts of the raw sequences 
...
gdrive_download 1TasxvzYEym3iBxMe4k8nnKd2bCVnO_Us adapters.txt

for file in *.fq
do
   echo "python2.7 remove_adapters.py adapters.txt ${file} 1"
done > trimm.sh

cat trimm.sh | parallel
...
megahit -m 0.75 -t 120 -1 all.pe.qc.fq.1 -2 all.pe.qc.fq.2 -r all.se.qc.fq -o all.Megahit.assembly
...
# FragGeneScan
FragGeneScan -s ~/metagenomic_analysis_chapter_4/4_FOR_ASSEMBLY/all.Megahit.assembly/final.contigs.fa -w 1 -o belen_MG_Megahit_genecalling_fgs -t complete -p 120
...
bowtie2-build ~/metagenomic_analysis/4_FOR_ASSEMBLY/all.Megahit.assembly/${REF} ${reference}_build/${reference}.build
...
do
 ...
 cat ${sample}.pe.qc.fq ${sample}.se.qc.fq > ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.all.qc.fq
 echo "processing ${sample}...}"

 bowtie2 -p 70 -x ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/final.contigs_build/final.contigs.build -q ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.all.qc.fq -S ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.sam 
 echo "sam file is done..."
 
 rm -rf ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.all.qc.fq

 samtools view -Sb ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.sam > ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.bam
 echo "bam file is done..."

 rm -rf ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.sam

 samtools view -c -f 4 ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.bam > ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.reads-unmapped.count.txt
 echo "unmapped reads info done..."

 samtools view -c -F 4 ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.bam > ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.reads-mapped.count.txt
 echo "mapped reads info done..."

 samtools sort -o ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.sorted.bam ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.bam
 echo "bam file was sorted..."

 rm -rf ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.bam

 samtools index ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.sorted.bam
 echo "soerted bam file was indexed..."

 samtools idxstats ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.sorted.bam > ~/metagenomic_analysis_chapter_4/6_SAMPLE_MAPPING/${sample}.reads.by.contigs.txt
...
cd ..
mkdir 8_ANNOTATION
cp ./7_NORMALISE_MAPPING/TABLE_normalised_per_sample.txt ./8_ANNOTATION/
cd ./8_ANNOTATION
...
python2.7 contig_mapping_to_genecall_mapping.py ~/metagenomic_analysis_chapter_4/5_FGS/belen_MG_Megahit_genecalling_fgs.faa TABLE_normalised_per_sample.txt
...
cp JGI_FUNGAL_PROTEINS_ANNOTATED_20210312.faa.zip /mnt/DATA/belen/8_ANNOTATION/
cd /mnt/DATA/belen/8_ANNOTATION/
unzip JGI_FUNGAL_PROTEINS_ANNOTATED_20210312.faa.zip

diamond blastp -d ~/metagenomic_analysis_chapter_4/8_ANNOTATION/JGI_FUNGAL_PROTEINS_ANNOTATED_20210312.faa -q ~/metagenomic_analysis_chapter_4/5_FGS/belen_MG_Megahit_genecalling_fgs.faa -e 1E-5 -o genecalling_JGI_FUN_20210312.txt -f 6 -p 120 -b12 -c1
...
# GENERA DEFINED
diamond blastp -d /mnt/DATA/DATABASES/NCBI_nr_DIAMOND/NCBI_nr_20210225_diamond_GENERA -q /mnt/DATA/belen/metagenomic_analysis_chapter_4/5_FGS/belen_MG_Megahit_genecalling_fgs.faa -e 1E-5 -o belen_MG_genecalling_NCBI_nr_PROTEINS_GENERA.txt -f 6 -p 120 -b12 -c1
...
python2.7 split_fasta_by_group_size.py /mnt/DATA/belen/metagenomic_analysis_chapter_4/5_FGS/belen_MG_Megahit_genecalling_fgs.faa 83000
...
do
  file=${i##*/}
  ko=${file%%.hmm}
  echo "hmmsearch --tblout ko_tbl/${ko}.out.txt --noali --cpu 1 -E 1e-5 ${i} ~/metagenomic_analysis_chapter_4/5_FGS/belen_MG_Megahit_genecalling_fgs.faa >/dev/null 2>&1"
...
awk -F'\t' '{print $3}' ~/metagenomic_analysis_chapter_4/9_SPLIT/CAZy_BEST_SIMPLE.txt | sort | uniq > CAZy_BEST_unique.txt
...
cd ../8_ANNOTATION/
mv TABLE_NORM_SAMPLES_GENECALL.txt ~/metagenomic_analysis/9_SPLIT/
cp TAXONOMY_BEST_OF_SIMPLE.txt ~/metagenomic_analysis/belen/9_SPLIT/
cd ../9_SPLIT

TABLE="~/metagenomic_analysis_chapter_4/9_SPLIT/TABLE_NORM_SAMPLES_GENECALL.txt"
...
python2.7 link_simple_table_to_mapping_table.py ${TABLE_BASE}_TAX_CAZy.txt ../8_ANNOTATION/FUNCTION_JGI_KOG_SIMPLE.txt KOG e-val ${TABLE_BASE}_TAX_CAZy_KOG.tab
...
cp ./9_SPLIT/TABLE_NORM_SAMPLES_GENECALL_TAX_CAZy_KOG_KEGG.tab ./10_FINAL_TABLES
cp ./9_SPLIT/TAX_TAB_FINAL.tab ./10_FINAL_TABLES
cp ./9_SPLIT/TABLE_NORM_SAMPLES_GENECALL_TAX2_CAZy_KOG_KEGG.tab ./10_FINAL_TABLES
